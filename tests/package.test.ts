...
test("published package loads in Pi without development or peer dependencies", async () => {
	...
	try {
		...
		await run([process.execPath, "pm", "pack", "--filename", archive, "--ignore-scripts"], root);
		await writeFile(join(directory, "package.json"), JSON.stringify({
			private: true,
			dependencies: { "@nijaru/pi-provider-compaction": `file:${archive}` },
		}));
		await run([process.execPath, "install", "--production", "--omit=peer", "--ignore-scripts"]);
		...
		const output = await run([
			...
			"--extension", join(directory, "node_modules/@nijaru/pi-provider-compaction/index.ts"),
			"--mode", "rpc",
		]);
